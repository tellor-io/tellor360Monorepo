...
async function deployTellor360(_network, _pk, _nodeURL, _tokenAddress, _reportingLock, _stakeAmountDollarTarget, _stakingTokenPrice,_minTokenstakeAmount, _stakingTokenPriceQueryId, _teamMultisigAddress, _autopayFee) {
    ...
    const flex = await flexfac.deploy(_tokenAddress, _reportingLock, _stakeAmountDollarTarget, _stakingTokenPrice, _minTokenstakeAmount,_stakingTokenPriceQueryId, { gasPrice:50000000000  })
    ...
    const governance = await govfac.deploy(flex.address, _teamMultisigAddress, { gasPrice:50000000000 })
    ...
    const qstorage = await qstoragefac.deploy({ gasPrice:50000000000  })
    ...
    const autopay = await autopayfac.deploy(flex.address, qstorage.address, _autopayFee,{ gasPrice:50000000000 }) // tellorOracleAddress, queryDataStorageAddress, autopayFee
    ...
    await flex.deployTransaction.wait(7)
      ...
      await flex.init(governance.address,{ gasPrice:50000000000 })
